              ...
              {active && messages.length > 0 && (
                <button
                  onClick={async () => {
                    ...
                    if (!lastAi) return;
                    try {
                      const res = await fetch("/api/vibe-trading/strategy", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                          action: "create",
                          userId: "default-user-id", // placeholder until Supabase auth hook integrated
                          data: {
                            strategy_name: active.title,
                            description: messages[0]?.content || "",
                            generated_python_code: `# Freqtrade Strategy generated by Kairos\n# Strategy: ${active.title}`,
                            backtest_metrics: lastAi.metrics || {},
                          },
                        }),
                      });
                      if (res.ok) alert("Strategy saved to Supabase successfully!");
                      else alert("Failed to save strategy.");
                    } catch {
                      alert("Error saving strategy.");
                    }
                  }}
                  className="px-2.5 py-1 rounded-lg border border-amber-400/30 bg-amber-400/10 font-mono text-[11px] text-amber-300 hover:bg-amber-400/20 transition-colors"
                >
                  💾 Save to Supabase
                </button>
              )}
